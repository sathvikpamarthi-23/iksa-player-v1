...
function ensureDefaultUser() {
  ...
  db.prepare(`
    INSERT INTO users (id, email, password_hash, display_name, avatar_url, subscription_tier)
    VALUES (?, ?, ?, ?, ?, ?)
...
app.post('/api/auth/register', async (req, res) => {
  ...
  if (existing && !existing.email_verified) {
    ...
    await sendOTPEmail(normalizedEmail, code, 'email_verify');
...
app.get('/api/tracks', optionalAuthMiddleware, (req, res) => {
  ...
  if (tab === 'recent' && userId) {
    sql = `SELECT DISTINCT tracks.* FROM tracks INNER JOIN listening_history ON listening_history.track_id = tracks.id WHERE listening_history.user_id = ? AND tracks.is_public = 1 AND tracks.deleted_at IS NULL ORDER BY listening_history.started_at DESC`;
    params.length = 0;
    params.push(userId);
  }
  ...
  const rows = db.prepare(`${sql} ORDER BY ${sortSql}, title COLLATE NOCASE ASC LIMIT ?`).all(...params, Math.max(1, Math.min(100, Number(limit) || 50)));
...
app.delete('/api/playlists/:playlistId', authMiddleware, (req, res) => {
  const result = db.prepare('DELETE FROM playlists WHERE id = ? AND user_id = ?').run(req.params.playlistId, req.user.id);
